...
reference_architecture_zh:
  title: "面向可验证智能体网络的八层参考架构"
  intro: "该架构将跨域企业智能体行动拆分为八个可验证、可治理且可独立实施的层。每一层回答一个不同的信任问题，并向持续的“采集与评估—策略决策—策略执行—运行时反馈”闭环提供输入。"
  caption: "表 1：面向跨域 B2B 与 B2G 智能体行动的八层参考架构。"
  layers:
    - name: "1. 治理与信任"
      components: "法律、域规则手册、互认机制、信任注册表、签发者资质认可和语义治理"
      outcome: "在自治信任域之间建立可被识别和接受的权威来源与授权关系。"
    - name: "2. 组织身份"
      components: "企业数字钱包、由权威登记册支持的企业身份、可验证法人机构标识符（vLEI）、角色、许可证、电子签名和电子印章"
      outcome: "跨司法管辖区验证法人身份及其代表权。"
    - name: "3. 智能体身份与授权委托"
      components: "智能体钱包、去中心化标识符（DID）或工作负载身份、授权委托书（PoA）和任务能力凭证"
      outcome: "将受限的技术代理能力与可问责的授权主体相连接。"
    - name: "4. AI 服务保障"
      components: "AI 服务护照（AISP）、测试、评估、验证与确认（TEVV）、风险概况、事件记录和监测"
      outcome: "验证系统来源及其当前保障状态。"
    - name: "5. 数据与产品证据"
      components: "数据空间、数字产品护照/数字电池护照（DPP/DBP）系统、来源信息、语义校验和证据图谱"
      outcome: "为请求的决策提供可验证的证据基础。"
    - name: "6. 决策与执行"
      components: "M-Trust 信任评估、策略决策点（PDP）、策略执行点（PEP）、连接器或网关"
      outcome: "实现由本地策略主导、上下文感知且具有时限的跨域授权。"
    - name: "7. 注册表与密码韧性"
      components: "可验证数据注册表（VDR）或受治理注册表、可选分布式账本、可信执行环境、远程证明、密码敏捷性和后量子密码（PQC）迁移走廊"
      outcome: "维护当前共享状态，以及持久的完整性、真实性和机密性。"
    - name: "8. 可问责性证据"
      components: "签名行动回执、时间戳、策略版本、证据快照和状态快照"
      outcome: "形成可重建的决策与执行历史。"
  transaction_flow: "端到端交易遵循七个步骤：智能体提出一项具体行动；网关请求授权与保障证据；智能体钱包出示身份与授权委托；验证者解析签发者信息、凭证状态、语义和证据引用；策略引擎计算有效权限并建立临时信任；策略执行点仅执行范围受限的能力；网关生成签名行动回执并更新持续监测状态。"
  scope_note: "本架构有意限定其主张范围：它是一项设计与标准化建议，并不构成生产性能、法律资格、互操作性或政治协调已经实现的证据。相关主张需要跨司法管辖区的公开技术配置文档、参考实现、一致性测试结果和试点验证。"
  terminology_note: "M-Trust 的“多方信任”“基于智能体意图感知的信任”和“跨域信任”沿用 GSMA 大中华区中文征求意见稿；“可验证凭证”“签发者”“验证者”和“可验证数据注册表”沿用 W3C 授权中文翻译；“去中心化标识符（DID）”沿用 W3C 中文术语。Spherity 提出的八层组合及其中文表述属于本研究的参考架构，不表示 GSMA 或 W3C 对该架构的认可。"
  terminology_sources:
    - name: "GSMA 大中华区：《面向未来智能体网络的信任范式演进（征求意见稿）》"
      url: "https://www.gsma.com/about-us/regions/greater-china/gsma_resources/%E9%9D%A2%E5%90%91%E6%9C%AA%E6%9D%A5%E6%99%BA%E8%83%BD%E4%BD%93%E7%BD%91%E7%BB%9C%E7%9A%84%E4%BF%A1%E4%BB%BB%E8%8C%83%E5%BC%8F%E6%BC%94%E8%BF%9B%EF%BC%88%E5%BE%81%E6%B1%82%E6%84%8F%E8%A7%81%E7%A8%BF/"
    - name: "W3C：《可验证凭证数据模型 v2.0》授权中文翻译"
      url: "https://www.w3.org/Translations/VC20-zh-CAT/Overview.html"
    - name: "W3C：去中心化标识符（DID）v1.1 中文说明"
      url: "https://www.w3.org/zh-hans/news/2026/w3c-invites-implementations-of-decentralized-identifiers-dids-v1-1/"
faq_schema: true
...
toc_items:
  ...
  - title: "八层参考架构（中文）"
    href: "#reference-architecture-zh"
